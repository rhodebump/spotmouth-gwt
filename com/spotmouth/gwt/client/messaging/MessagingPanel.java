    ...
    private void doSearch() {
        ...
        myService.searchMessages(messageRequest, new AsyncCallback() {
            ...
            public void onSuccess(Object result) {
                ...
                resultsUL.clear();
                addItems4(mobileResponse.getSearchMessages(),resultsUL);
                GWT.log("mobileResponse.getSearchMessages() " + mobileResponse.getSearchMessages().size());
...
//            resultsUL.getElement().setId("mp_message");
            ...
            addItems4(mywebapp.getMessages().getInMessages(), inUL);
            addItems4(mywebapp.getMessages().getSentMessages(), sentUL);
            addItems4(mywebapp.getMessages().getDraftMessages(), draftUL);
            refresh();
...
//        addItems2(mywebapp.getMessages().getDraftMessages(), "Draft", editMessageHandler);
    ...
    /*
    the message search returns itemholders, but we use notificationholders to build ui
    this utility method will wrapper itemholders for the ui
     */
    private void addItems3(List<ItemHolder> items, ULPanel ulPanel) {
        List<NotificationHolder> list = new ArrayList<NotificationHolder>();
        for (ItemHolder itemHolder : items) {
            NotificationHolder notificationHolder = new NotificationHolder();
            notificationHolder.setItemHolder(itemHolder);
            list.add(notificationHolder);
        }
        addItems4(list, ulPanel);
    }

    private void addItems4(List<NotificationHolder> items, ULPanel ulPanel) {
        for (NotificationHolder notificationHolder : items) {
            ItemHolder itemHolder = notificationHolder.getItemHolder();
            ...
            MessageResultComposite messageResultComposite = new MessageResultComposite(imageUser, notificationHolder, this);
...
//                imageUser = new Image(MyWebApp.resources.spot_image_placeholder130x130());
//            }
//            MessageResultComposite messageResultComposite = new MessageResultComposite(imageUser,itemHolder,this);
//            messageResultComposite.setMessageText(itemHolder.getTextData());
//            messageResultComposite.setTimeReceived(itemHolder.getCreateDateDisplay());
//            if (itemHolder.getUserHolder() != null) {
//                messageResultComposite.setUsername(itemHolder.getUserHolder().getUsername());
...
//    };
    ...
    public void reportSpam(final NotificationHolder notificationHolder, final MessageResultComposite mrc) {
        getMessagePanel().clear();
        MessageRequest messageRequest = new MessageRequest();
        messageRequest.setNotificationHolder(notificationHolder);
        messageRequest.setAuthToken(mywebapp.getAuthToken());
        ApiServiceAsync myService = mywebapp.getApiServiceAsync();
        myService.reportMessageSpam(messageRequest, new AsyncCallback() {
            public void onFailure(Throwable caught) {
                //postDialog.hide();
                getMessagePanel().displayError(caught.getMessage());
            }
            ...
            public void onSuccess(Object result) {
                ...
                MobileResponse mobileResponse = (MobileResponse) result;
                if (mobileResponse.getStatus() == 1) {
                    getMessagePanel().displayMessage("Spam reported");
                    //what should we do here, remove this from our local list of messages, or re-init from server
                    mrc.setVisible(false);
                    removeNotification(notificationHolder);
                } else {
                    getMessagePanel().displayErrors(mobileResponse.getErrorMessages());
                }
            }
    ...
    //this will call to server to delete the message, and just do a hide of the component
    public void deleteMessage(final NotificationHolder notificationHolder, final MessageResultComposite mrc) {
        getMessagePanel().clear();
        MessageRequest messageRequest = new MessageRequest();
        messageRequest.setNotificationHolder(notificationHolder);
        messageRequest.setAuthToken(mywebapp.getAuthToken());
        ApiServiceAsync myService = mywebapp.getApiServiceAsync();
        myService.deleteMessage(messageRequest, new AsyncCallback() {
            public void onFailure(Throwable caught) {
                //postDialog.hide();
                getMessagePanel().displayError(caught.getMessage());
            }
            ...
            public void onSuccess(Object result) {
                ...
                MobileResponse mobileResponse = (MobileResponse) result;
                if (mobileResponse.getStatus() == 1) {
                    getMessagePanel().displayMessage("Message deleted");
                    //what should we do here, remove this from our local list of messages, or re-init from server
                    mrc.setVisible(false);
                    //let's sweep through and remove this from our local store
                    removeNotification(notificationHolder);
    ...
    private void removeNotification(NotificationHolder notificationHolder) {
        mywebapp.getMessages().getInMessages().remove(notificationHolder);
        mywebapp.getMessages().getSentMessages().remove(notificationHolder);
        mywebapp.getMessages().getDraftMessages().remove(notificationHolder);
